...
if [[ -z "$manifest_path" ]]; then
  ...
  helm template reinstall-a "$chart_archive" "${render_args[@]}" > "$tmp/a.yaml"
...
jq -e '.schemaVersion == 1 and
  (((.images | length) == 5 and (.childCharts | length) == 4) or
   ((.images | length) == 6 and (.childCharts | length) == 5) or
