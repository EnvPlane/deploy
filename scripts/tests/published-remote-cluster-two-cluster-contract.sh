...
bash -n "$script"
for required in 'helm upgrade --install' '/api/v1/remote-clusters' 'control-plane-preflight' 'resource-scan/start' 'helm-direct/preflight' 'creates a real full environment through the UI' 'unreachable.invalid' '/rotate' '/repair'; do grep -Fq "$required" "$script"; done
