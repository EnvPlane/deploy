...
bash -n "$script"
for required in 'helm upgrade --install' '/api/v1/management-endpoint-profile' 'managementCopy' 'ENVPILOT_E2E_REMOTE_CONTROL_PLANE_CA_FILE' 'ENVPILOT_E2E_REMOTE_CONTROL_PLANE_ROTATED_CA_FILE' 'control-plane-preflight' 'assert_remote_runtime_after_installer_exit' 'resource-scan/start' 'helm-direct/preflight' 'creates a real full environment through the UI' 'does-not-resolve.invalid' 'private CA rotation did not advance target trust revision' '/repair'; do grep -Fq "$required" "$script"; done
