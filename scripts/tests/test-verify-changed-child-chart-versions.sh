...
tmp="$(mktemp -d "${TMPDIR:-/tmp}/envpilot-chart-version-test.XXXXXX")"
trap 'rm -rf "$tmp"' EXIT

git init -q "$tmp"
git -C "$tmp" config user.email test@example.invalid
git -C "$tmp" config user.name chart-version-test
mkdir -p "$tmp/deploy/helm/envpilot-control-plane/templates"
printf 'apiVersion: v2\nname: envpilot-control-plane\nversion: 0.3.22\n' >"$tmp/deploy/helm/envpilot-control-plane/Chart.yaml"
printf 'first\n' >"$tmp/deploy/helm/envpilot-control-plane/templates/config.yaml"
git -C "$tmp" add .
...
sed -i.bak 's/version: 0.3.22/version: 0.3.23/' "$tmp/deploy/helm/envpilot-control-plane/Chart.yaml"
rm -f "$tmp/deploy/helm/envpilot-control-plane/Chart.yaml.bak"
git -C "$tmp" add .
