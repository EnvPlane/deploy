...
: "${ENVPLANE_E2E_MANAGEMENT_CONTEXT:?set management kube context}"
: "${ENVPLANE_E2E_TARGET_CONTEXT:?set target kube context}"
: "${ENVPLANE_E2E_UMBRELLA_REF:?set published OCI umbrella ref}"
: "${ENVPLANE_E2E_UMBRELLA_VERSION:?set immutable published umbrella version}"
: "${ENVPLANE_E2E_VALUES_FILE:?set management umbrella values file}"
: "${ENVPLANE_E2E_REMOTE_KUBERNETES_ENDPOINT:?set target Kubernetes HTTPS endpoint}"
: "${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL:?set stable target-pod-reachable HTTPS control-plane endpoint}"
: "${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_TLS_SERVER_NAME:?set private endpoint TLS server name}"
: "${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_FILE:?set private endpoint CA PEM file}"
: "${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_ROTATED_CA_FILE:?set rotated private endpoint CA PEM file}"
: "${ENVPLANE_E2E_HELM_CHART_REF:?set target-runner-resolvable OCI or HTTPS workload chart}"
: "${ENVPLANE_E2E_API_URL:?set stable management API URL for the E2E client}"
: "${ENVPLANE_E2E_UI_URL:?set stable management UI URL for browser E2E}"

namespace="${ENVPLANE_E2E_NAMESPACE:-envplane}"
release="${ENVPLANE_E2E_RELEASE:-envplane}"
project_id="${ENVPLANE_E2E_PROJECT_ID:-published-remote-e2e}"
cluster_id="${ENVPLANE_E2E_REMOTE_CLUSTER_ID:-$project_id}"
...
feature_namespace="${ENVPLANE_E2E_FEATURE_NAMESPACE:-envplane-e2e-feature}"
environment_id="${ENVPLANE_E2E_ENVIRONMENT_ID:-${project_id}-full}"
credential_mode="${ENVPLANE_E2E_REMOTE_CREDENTIAL_MODE:-existing}" # existing|submit
remote_cluster_create_mode="${ENVPLANE_E2E_REMOTE_CLUSTER_CREATE_MODE:-api}" # api|ui
credential_secret="${ENVPLANE_E2E_REMOTE_CREDENTIAL_SECRET:-${cluster_id}-kubeconfig}"
credential_key="${ENVPLANE_E2E_REMOTE_CREDENTIAL_KEY:-kubeconfig}"
credential_file="${ENVPLANE_E2E_REMOTE_CREDENTIAL_FILE:-}"
api_url="${ENVPLANE_E2E_API_URL%/}"
ui_url="${ENVPLANE_E2E_UI_URL%/}"
control_plane_ca_secret="${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_SECRET:-${cluster_id}-management-ca}"
control_plane_ca_key="${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_KEY:-ca.crt}"
target_control_plane_ca_secret="${ENVPLANE_E2E_REMOTE_TARGET_CA_SECRET:-${cluster_id}-control-plane-ca}"
target_control_plane_ca_key="${ENVPLANE_E2E_REMOTE_TARGET_CA_KEY:-$control_plane_ca_key}"
scm_provider="${ENVPLANE_E2E_SCM_PROVIDER:-gitlab}"
app_repo="${ENVPLANE_E2E_APP_REPOSITORY_URL:-}"
gitops_repo="${ENVPLANE_E2E_GITOPS_REPOSITORY_URL:-}"
app_branch="${ENVPLANE_E2E_APP_DEFAULT_BRANCH:-main}"
gitops_branch="${ENVPLANE_E2E_GITOPS_DEFAULT_BRANCH:-main}"
scm_token_file="${ENVPLANE_E2E_SCM_TOKEN_FILE:-}"
feature_ref="${ENVPLANE_E2E_FEATURE_REF:-201}"
runtime_stability_wait_seconds="${ENVPLANE_E2E_REMOTE_RUNTIME_STABILITY_WAIT_SECONDS:-35}"

for bin in curl helm jq kubectl; do command -v "$bin" >/dev/null || { echo "missing $bin" >&2; exit 2; }; done
[[ "$credential_mode" == "existing" || "$credential_mode" == "submit" ]] || { echo "ENVPLANE_E2E_REMOTE_CREDENTIAL_MODE must be existing or submit" >&2; exit 2; }
[[ "$remote_cluster_create_mode" == "api" || "$remote_cluster_create_mode" == "ui" ]] || { echo "ENVPLANE_E2E_REMOTE_CLUSTER_CREATE_MODE must be api or ui" >&2; exit 2; }
...
[[ "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL" =~ ^https:// ]] || { echo "remote control-plane endpoint must be explicit HTTPS" >&2; exit 2; }
case "${ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL,,}" in *host.minikube.internal*|*localhost*|*127.0.0.1*|*.svc/*|*.svc:*) echo "remote endpoint must be target-pod-reachable, not local or Service DNS" >&2; exit 2;; esac
[[ "$ENVPLANE_E2E_HELM_CHART_REF" == oci://* || "$ENVPLANE_E2E_HELM_CHART_REF" == https://* ]] || { echo "feature chart must be OCI or HTTPS" >&2; exit 2; }
[[ -n "$app_repo" && -n "$gitops_repo" ]] || { echo "set app and GitOps repository URLs" >&2; exit 2; }
[[ -n "$credential_file" && -r "$credential_file" ]] || { echo "set readable ENVPLANE_E2E_REMOTE_CREDENTIAL_FILE" >&2; exit 2; }
[[ -r "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_FILE" ]] || { echo "set readable ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_FILE" >&2; exit 2; }
[[ -r "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_ROTATED_CA_FILE" ]] || { echo "set readable ENVPLANE_E2E_REMOTE_CONTROL_PLANE_ROTATED_CA_FILE" >&2; exit 2; }
...
helm upgrade --install "$release" "$ENVPLANE_E2E_UMBRELLA_REF" --version "$ENVPLANE_E2E_UMBRELLA_VERSION" \
  --kube-context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" --namespace "$namespace" --create-namespace --values "$ENVPLANE_E2E_VALUES_FILE" --wait --timeout 15m
...
leader_lease="envplane-remote-cluster-reconciler"
leader_identity=""
for _ in $(seq 1 60); do
  leader_identity="$(kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" -n "$namespace" get lease "$leader_lease" -o json 2>/dev/null | jq -r '.spec.holderIdentity // ""' || true)"
...
kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" -n "$namespace" create secret generic "$control_plane_ca_secret" \
  --from-file="$control_plane_ca_key=$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_FILE" --dry-run=client -o yaml | \
  kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" apply -f - >/dev/null
management_profile="$(jq -n \
  --arg endpoint "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL" \
  --arg server_name "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_TLS_SERVER_NAME" \
...
jq -e --arg endpoint "$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL" '
...
if [[ "$credential_mode" == "existing" ]]; then
  kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" -n "$namespace" create secret generic "$credential_secret" --from-file="$credential_key=$credential_file" --dry-run=client -o yaml | kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" apply -f - >/dev/null
fi
remote_payload="$(jq -n --arg id "$cluster_id" --arg endpoint "$ENVPLANE_E2E_REMOTE_KUBERNETES_ENDPOINT" --arg secret "$credential_secret" --arg key "$credential_key" --arg ns "$namespace" --arg source_secret "$control_plane_ca_secret" --arg source_key "$control_plane_ca_key" --arg target_secret "$target_control_plane_ca_secret" --arg target_key "$target_control_plane_ca_key" --arg project "$project_id" --arg runtime_ns "$remote_namespace" --arg base "$base_namespace" --arg feature "$feature_namespace" '{id:$id,name:$id,kubernetes:{endpoint:$endpoint,credential_secret_ref:{name:$secret,namespace:$ns,key:$key}},control_plane:{trust:{mode:"managementCopy",source_secret_ref:{name:$source_secret,namespace:$ns,key:$source_key},target_secret_ref:{name:$target_secret,namespace:$runtime_ns,key:$target_key}}},agent:{enabled:true,release_name:("envplane-remote-"+$id+"-agent"),namespace:$runtime_ns,project_id:$project},runner:{enabled:true,release_name:("envplane-remote-"+$id+"-runner"),namespace:$runtime_ns,project_id:$project},discovery:{allowed_namespaces:[$base,$feature]},feature_namespaces:{mode:"shared",shared_namespace:$feature,allowed_prefixes:["envplane-e2e-"]}}')"
if [[ "$remote_cluster_create_mode" == "ui" ]]; then
	(cd "$(dirname "$0")/../../frontend" && PLAYWRIGHT_SKIP_WEBSERVER=1 PLAYWRIGHT_TEST_DIR=./tests/e2e-real PLAYWRIGHT_BASE_URL="$ui_url" ENVPLANE_E2E_API_URL="$api_url" ENVPLANE_E2E_REAL_CLUSTER=1 ENVPLANE_E2E_REMOTE_CLUSTER_UI=1 ENVPLANE_E2E_REMOTE_CLUSTER_ID="$cluster_id" ENVPLANE_E2E_REMOTE_KUBERNETES_ENDPOINT="$ENVPLANE_E2E_REMOTE_KUBERNETES_ENDPOINT" ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL="$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_URL" ENVPLANE_E2E_REMOTE_CONTROL_PLANE_TLS_SERVER_NAME="$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_TLS_SERVER_NAME" ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_SECRET="$control_plane_ca_secret" ENVPLANE_E2E_REMOTE_CONTROL_PLANE_CA_KEY="$control_plane_ca_key" ENVPLANE_E2E_REMOTE_TARGET_CA_SECRET="$target_control_plane_ca_secret" ENVPLANE_E2E_REMOTE_TARGET_CA_KEY="$target_control_plane_ca_key" ENVPLANE_E2E_REMOTE_CREDENTIAL_SECRET="$credential_secret" ENVPLANE_E2E_REMOTE_CREDENTIAL_KEY="$credential_key" ENVPLANE_E2E_REMOTE_CREDENTIAL_NAMESPACE="$namespace" ENVPLANE_E2E_REMOTE_RUNTIME_NAMESPACE="$remote_namespace" ENVPLANE_E2E_BASE_NAMESPACE="$base_namespace" ENVPLANE_E2E_FEATURE_NAMESPACE="$feature_namespace" npm run test:e2e -- --grep 'creates a managed remote cluster through the UI')
else
...
' >"$tmp/healthy.json"
kubectl --context "$ENVPLANE_E2E_TARGET_CONTEXT" -n "$remote_namespace" get secret "$target_control_plane_ca_secret" -o json | jq -e \
  --arg cluster "$cluster_id" '.metadata.labels["envplane.io/managed-by"] == "remote-cluster-reconciler" and .metadata.labels["envplane.io/remote-cluster-id"] == $cluster and .metadata.labels["envplane.io/purpose"] == "control-plane-trust"' >/dev/null
for deployment in "envplane-remote-${cluster_id}-agent" "envplane-remote-${cluster_id}-runner"; do
  kubectl --context "$ENVPLANE_E2E_TARGET_CONTEXT" -n "$remote_namespace" rollout status "deployment/$deployment" --timeout=5m
  kubectl --context "$ENVPLANE_E2E_TARGET_CONTEXT" -n "$remote_namespace" get pod -l "app.kubernetes.io/instance=$deployment" -o json | jq -e '[.items[].status.initContainerStatuses[]? | select(.name=="control-plane-preflight") | .state.terminated.exitCode] | all(. == 0)' >/dev/null
done
...
assert_remote_runtime_after_installer_exit
scm_token="$(awk '/^(glpat-|ghp_|github_pat_)/{print; exit}' "${ENVPLANE_E2E_SCM_TOKEN_FILE:-/dev/null}")"
[[ -n "$scm_token" ]] || { echo "set ENVPLANE_E2E_SCM_TOKEN_FILE for bootstrap validation" >&2; exit 2; }
...
bootstrap_patch="$(jq -n --arg base "$base_namespace" --arg ref "$ENVPLANE_E2E_HELM_CHART_REF" --arg feature "$feature_namespace" '{step_data:{selectedBaseNamespaces:[$base],deployment:{backend:"helm_direct",helmDirect:{chartRef:$ref,namespaceMode:"shared",namespacePattern:$feature,releaseNamePattern:"envplane-e2e",wait:true,createNamespace:false}}}}')"
...
(cd "$(dirname "$0")/../../frontend" && PLAYWRIGHT_SKIP_WEBSERVER=1 PLAYWRIGHT_TEST_DIR=./tests/e2e-real PLAYWRIGHT_BASE_URL="$ui_url" ENVPLANE_E2E_API_URL="$api_url" ENVPLANE_E2E_REAL_CLUSTER=1 ENVPLANE_E2E_RUN_LIFECYCLE=1 ENVPLANE_E2E_KEEP_ENVIRONMENT=1 ENVPLANE_E2E_PROJECT_ID="$project_id" ENVPLANE_E2E_ENVIRONMENT_ID="$environment_id" npm run test:e2e -- --grep 'creates a real full environment through the UI')
...
helm --kube-context "$ENVPLANE_E2E_TARGET_CONTEXT" -n "$target_namespace" status "$release_name" >/dev/null
control_release="$(helm --kube-context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" list --all-namespaces -o json | jq -r --arg release "$release_name" '.[] | select(.name == $release) | .name')"
...
kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" -n "$namespace" create secret generic "$control_plane_ca_secret" \
  --from-file="$control_plane_ca_key=$ENVPLANE_E2E_REMOTE_CONTROL_PLANE_ROTATED_CA_FILE" --dry-run=client -o yaml | \
  kubectl --context "$ENVPLANE_E2E_MANAGEMENT_CONTEXT" apply -f - >/dev/null
