...
release="${AUTH_MANAGED_SECRET_SMOKE_RELEASE:-envpilot-auth-smoke}"
namespace="${AUTH_MANAGED_SECRET_SMOKE_NAMESPACE:-envpilot-auth-smoke}"
...
deployment="envpilot-control-plane"
...
test "$(kubectl --context "$AUTH_MANAGED_SECRET_SMOKE_CONTEXT" --namespace "$namespace" get deployment "$deployment" -o jsonpath='{.spec.template.spec.containers[?(@.name=="api")].env[?(@.name=="ENVPILOT_AUTHENTICATION_MANAGED_SECRET_NAME")].value}')" = "$secret"
