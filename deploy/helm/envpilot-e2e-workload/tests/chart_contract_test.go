...
func TestE2EWorkloadUsesTheStandardImageContract(t *testing.T) {
	...
	tagRender := render(
		"--set", "image.repository=registry.example.internal/envpilot/e2e",
	...
	for _, expected := range []string{
		`image: "registry.example.internal/envpilot/e2e:build-20260731"`,
	...
	if !strings.Contains(digestRender, `image: "registry.example.internal/envpilot/e2e@`+digest+`"`) ||
		strings.Contains(digestRender, `image: "registry.example.internal/envpilot/e2e:must-not-be-rendered"`) {
