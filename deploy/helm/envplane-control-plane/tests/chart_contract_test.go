...
func TestControlPlaneChartDefinesAPIDeploymentAndService(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"ENVPLANE_REDIS_URL",
		"ENVPLANE_CREDENTIAL_ENCRYPTION_KEY",
	...
	for _, expected := range []string{
		...
		"ENVPLANE_DEPENDENCY_WAIT_TIMEOUT_SECONDS",
...
func TestControlPlaneChartManagesCredentialEncryptionKey(t *testing.T) {
	...
	for _, expected := range []string{
		"name: envplane-control-plane-runtime-credentials",
		"envplane.io/purpose: credential-encryption",
		"name: ENVPLANE_CREDENTIAL_ENCRYPTION_KEY",
...
func TestControlPlaneChartUsesWritableDataAndGitOpsPathsFromEnv(t *testing.T) {
	rendered := renderControlPlaneChart(t,
		...
		"--set", "env.ENVPLANE_GITOPS_DIR=/custom/envplane-data/gitops",
	)
	for _, expected := range []string{
		"mountPath: /var/lib/envplane/data",
		"name: ENVPLANE_DATA_DIR",
		`value: "/custom/envplane-data"`,
		"name: ENVPLANE_GITOPS_DIR",
		`value: "/custom/envplane-data/gitops"`,
	} {
		if !strings.Contains(rendered, expected) {
...
func TestControlPlaneChartUsesWriteOnlyOAuthSecretReferences(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"name: ENVPLANE_GITLAB_OAUTH_CLIENT_SECRET",
		"name: \"envplane-oauth\"",
	...
	if strings.Contains(rendered, "kind: Secret\nmetadata:\n  name: envplane-oauth") || strings.Contains(rendered, "kind: Secret\nmetadata:\n  name: \"envplane-oauth\"") {
...
func TestControlPlaneChartDisablesOAuthByDefaultAndValidatesLegacyMode(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		{"--set", "auth.existingSecret=envplane-oauth"},
	} {
...
func TestControlPlaneChartPropagatesCanonicalPublicURL(t *testing.T) {
	rendered := renderControlPlaneChart(t,
		"--set", "global.envplane.publicURL=https://global.example.test",
		"--set", "publicURL=https://child.example.test",
	)
	if !strings.Contains(rendered, "name: ENVPLANE_PUBLIC_URL\n              value: \"https://child.example.test\"") {
...
func TestControlPlaneChartUsesServiceDNSForSameClusterAgentBootstrapAndAllowsRemoteOverride(t *testing.T) {
	...
	for _, expected := range []string{
		"name: ENVPLANE_AGENT_CONTROL_PLANE_URL",
		`value: "http://envplane-control-plane.envplane.svc:8080"`,
		"name: ENVPLANE_AGENT_IMAGE_REPOSITORY",
		`value: "ghcr.io/envplane/agent"`,
		"name: ENVPLANE_AGENT_IMAGE_TAG",
		`value: "0.1.4"`,
	} {
	...
	remote := renderControlPlaneChart(t,
		"--namespace", "envplane",
		"--set", "agentBootstrap.controlPlaneURL=https://api.envplane.example.com",
		...
		"--set", "agentBootstrap.chart.ref=oci://registry.example/envplane-agent",
		"--set", "agentBootstrap.chart.version=0.2.7",
	)
	for _, expected := range []string{
		`value: "https://api.envplane.example.com"`,
		`value: "oci://registry.example/envplane-agent"`,
		`value: "0.2.7"`,
		...
		`name: ENVPLANE_CONTROL_PLANE_CLUSTER_ID`,
	...
	if !strings.Contains(remote, `value: "https://api.envplane.example.com"`) {
		t.Fatalf("remote Agent endpoint override was not rendered:\n%s", remote)
	}
	umbrellaRemote := renderControlPlaneChart(t,
		"--namespace", "envplane",
		"--set", "global.envplane.remoteControlPlane.endpoint=https://api.umbrella.example.com",
		"--set", "global.envplane.remoteControlPlane.tls.caSecretRef.name=umbrella-remote-ca",
		"--set", "global.envplane.remoteControlPlane.tls.caSecretRef.key=private-ca.crt",
	)
	for _, expected := range []string{
	...
	legacyRemote := renderControlPlaneChart(t,
		"--namespace", "envplane",
		"--set", "env.ENVPLANE_AGENT_CONTROL_PLANE_URL=https://legacy-api.envplane.example.com",
	)
	if strings.Count(legacyRemote, "name: ENVPLANE_AGENT_CONTROL_PLANE_URL") != 1 || !strings.Contains(legacyRemote, `value: "https://legacy-api.envplane.example.com"`) {
...
func TestControlPlaneChartUsesNamespaceScopedSecretReaderInsteadOfClusterAdmin(t *testing.T) {
	rendered := renderControlPlaneChart(t,
		...
		"--set", "rbac.secretReader.namespaces[0]=envplane-secrets",
		"--set", "rbac.secretReader.namespaces[1]=shared-secrets",
	)
	for _, expected := range []string{
		"kind: Role",
		"kind: RoleBinding",
		"name: envplane-control-plane-secret-reader",
		"namespace: \"envplane-secrets\"",
...
func TestControlPlaneChartCreatesNameScopedAuthenticationManagedSecret(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, expected := range []string{
				...
				"envplane.io/managed-secret: authentication",
				"envplane.io/purpose: authentication-managed-store",
				"name: envplane-control-plane-authentication-managed-secret",
				"resourceNames: [\"" + test.secretName + "\"]",
				"verbs: [\"get\", \"update\", \"patch\"]",
				"name: ENVPLANE_AUTHENTICATION_MANAGED_SECRET_NAME\n              value: \"" + test.secretName + "\"",
			} {
				if !strings.Contains(rendered, expected) {
	...
	for _, forbidden := range []string{
		...
		"resourceNames: [\"envplane-control-plane-authentication\"]\n    verbs: [\"get\", \"list\"]",
	} {
		if strings.Contains(rendered, forbidden) {
	...
	secretStart := strings.Index(rendered, "name: envplane-control-plane-authentication\n  namespace: \"management-system\"")
	if secretStart < 0 {
...
func TestControlPlaneChartUsesPodNamespaceForRemoteClusterLeaderElection(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"name: envplane-control-plane-remote-cluster-reconciler\n  namespace: \"management-system\"",
		"resources: [\"leases\"]",
		"verbs: [\"get\", \"create\", \"update\", \"patch\"]",
	} {
		if !strings.Contains(rendered, expected) {
	...
	if strings.Contains(rendered, "ENVPLANE_REMOTE_CLUSTER_RECONCILER_NAMESPACE") {
...
func TestControlPlaneChartRendersExplicitLeaderElectionNamespaceAndMatchingRBAC(t *testing.T) {
	...
	for _, expected := range []string{
		"name: ENVPLANE_REMOTE_CLUSTER_RECONCILER_NAMESPACE\n              value: \"lease-system\"",
...
func TestControlPlaneChartSupportsExistingServiceAccountAndExternalRBAC(t *testing.T) {
	...
	cmd := exec.Command("helm", "template", "envplane", chartPath, "--set", "serviceAccount.create=false", "--set", "postgres.auth.existingSecret=chart-test-postgres", "--set", "postgres.tls.enabled=false")
	cmd.Dir = chartPath
...
func TestControlPlaneChartRendersIngressForFrontendAndAPIWhenExplicitlyEnabled(t *testing.T) {
	rendered := renderControlPlaneChart(t,
		...
		"--set", "ingress.domain=demo.envplane.example.com",
	...
	for _, expected := range []string{
		...
		`host: "demo.envplane.example.com"`,
		`- "demo.envplane.example.com"`,
...
func TestControlPlaneChartWiresPostgresRedisAndMigrations(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"name: envplane-control-plane-redis",
		"image: \"postgres:16-alpine\"",
		"image: \"redis:7-alpine\"",
		"name: ENVPLANE_DATABASE_URL",
		"name: ENVPLANE_REDIS_URL",
		"name: wait-postgres",
		"name: wait-redis",
		`value: "redis://:$(REDIS_PASSWORD)@envplane-control-plane-redis:6379/0"`,
		"--requirepass",
		"envplane-control-plane-redis-auth",
		`value: "/var/lib/envplane/migrations/postgres"`,
		"--appendonly",
...
func TestControlPlaneChartUsesCanonicalFrontendDependency(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"repository: file://../envplane-frontend",
...
func renderControlPlaneChart(t *testing.T, args ...string) string {
	t.Helper()
	chartPath := controlPlaneChartPath(t)
	commandArgs := append([]string{"template", "envplane", chartPath, "--set", "postgres.auth.existingSecret=chart-test-postgres", "--set", "postgres.tls.enabled=false"}, args...)
...
func buildControlPlaneDependencies(t *testing.T) {
	...
	buildControlPlaneDependenciesOnce.Do(func() {
		...
		fixtureRoot, err := os.MkdirTemp("", "envplane-control-plane-chart-")
		if err != nil {
			t.Fatalf("create temporary fixture: %v", err)
		}
		controlPlaneChartFixture = filepath.Join(fixtureRoot, "envplane-control-plane")
		copyChartTree(t, source, controlPlaneChartFixture)
		copyChartTree(t, filepath.Join(filepath.Dir(source), "envplane-frontend"), filepath.Join(fixtureRoot, "envplane-frontend"))
