...
{{- define "envplane.firstStartRegistrationSecretName" -}}
{{- $global := default (dict) .Values.global -}}
...
{{- if eq $mode "existing" -}}
{{- required "global.envplane.firstStartRegistration.existingSecret is required when mode=existing" $registration.existingSecret -}}
{{- else -}}
...
*/}}
{{- define "envplane.registryImagePullSecrets" -}}
{{- $envplane := include "envplane.globalConfig" . | fromJson -}}
...
*/}}
{{- define "envplane.effectivePlatformDependencies" -}}
...
{{- if $autoAccess -}}
  {{- $smoke := dict "serviceName" (default "envplane-frontend" (get (default (dict) (get $access "services")) "frontendName")) "namespace" .Release.Namespace "port" 3000 "host" (default "" (get $accessIngress "host")) -}}
...
*/}}
{{- define "envplane.platformReconcilerStatusConfigMapName" -}}
{{- $envplane := include "envplane.globalConfig" . | fromJson -}}
...
*/}}
{{- define "envplane.remoteClusterCompatibilityConfigMapName" -}}
...
{{- if and (eq $mode "managed") (ne (default "external" $dependency.ownership) "envplane") -}}{{ fail "platformDependencies ownership must be envplane when mode=managed" }}{{- end -}}
