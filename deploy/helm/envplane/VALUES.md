...
|---|---|---|
| Control plane | `controlPlane.enabled` | `envplane-control-plane.*` |
| Frontend | `frontend.enabled` | `envplane-frontend.*` |
| Agent | `agent.enabled` | `envplane-agent.*` |
| Runner | `runner.enabled` | `envplane-runner.*` |
...
image reference plus `envplane.io/source-revision` and `envplane.io/release` so
operators can trace a workload to its CI build without using mutable tags.

`envplane-control-plane` and `envplane-frontend` also expose a Kubernetes
...
global:
  envplane:
    registry:
      mode: existing
      existingSecret: envplane-registry
      preflight:
        enabled: true
```
...
`envplane-control-plane.auth.managedSecret.nameOverride`). It contains no OAuth
...
Set `global.envplane.publicURL` to the canonical public HTTPS origin before
...
the two child charts and select `global.envplane.firstStartRegistration.mode`:
...
`envplane-control-plane.agentBootstrap.controlPlaneURL` empty. The control
plane then generates bootstrap instructions using its Kubernetes Service FQDN
(`http://envplane-control-plane.<release-namespace>.svc:8080`), rather than a
  ...
  envplane:
    remoteControlPlane:
      ...
      tls:
        ...
        caSecretRef: {name: envplane-remote-ca, key: ca.crt}
```

The endpoint may be private or public DNS, but must be target-pod-reachable
HTTPS. `localhost`,
`envplane.local`, `host.minikube.internal`, port-forwards and Kubernetes
...
`envplane-control-plane.agentBootstrap.chart.ref` and `.version`. The umbrella
release sets these from its `envplane-agent` dependency, records the exact value
...
global:
  envplane:
    firstStartRegistration:
      mode: existing
      existingSecret: platform-envplane-first-start
      cluster:
        id: management-cluster
```
...
`envplane-control-plane.postgres` and `.redis` use `enabled` as the legacy
...
the shared `global.envplane.registry.mode=existing` Secret or explicit
...
through `envplane-control-plane.rbac.secretReader.namespaces`. It has no
ClusterRole. Agent discovery is read-only; set
`envplane-agent.rbac.discovery.scope=cluster` explicitly only when namespace
...
|---|---|
| `images.api.*` | `envplane-control-plane.image.*` |
| `images.frontend.*` | `envplane-frontend.image.*` |
...
helm lint deploy/helm/envplane -f values.yaml
helm template envplane deploy/helm/envplane -f values.yaml > rendered.yaml
```
